  ...
  if (jobsInProgress < MAX_JOBS) {
    store.dequeue('job').then(function (jobOptions) {
      ...
      worker.on('message', m => {
        ...
        if (m.status === 'success') { // Did the linter complete?
          if (m.results.compat.length) {
            store.increment('tests_failed');
            m.results.compat
              .map(c => c.message) // take all the compat warnings
              .reduce((unique, m) => { // get a unique list
                if (unique.indexOf(m) === -1) {
                  unique.push(m);
                }
                return unique;
              }, [])
              .forEach(m => store.increment('compat-error:' + m));
          } else {
